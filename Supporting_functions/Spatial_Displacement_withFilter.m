function [raw_spatialDisp, Map, mean_loc_prec, Count_Map] = Spatial_Displacement_withFilter(Store_track, Store_mask, Store_fits, Index, pixel_size, min_steps, gap_y, aspt_ratio, thres_ratio, uncertainty, fig, symmetrize, clr_map)
% Spatial_Displacement_withFilter Generates a spatial displacement map with an embedded
% localization precision filter.
%
% [raw_spatialDisp, htMap, mean_loc_prec] = Spatial_Displacement_withFilter(Store_track, Store_mask, Store_fits, Index, pixel_size, min_steps, gap_y, aspt_ratio, thres_ratio, uncertainty, fig, symmetrize, clr_map)
%
% This function processes multiple ROIs (given as cell arrays) to compute the
% normalized displacement map of tracking data. First, it applies a localization precision
% filter using the "goodfit" flag and confidence interval (widthcCI) from the fitted data.
% Then, standard procedures extract single‐cell masks and localization coordinates, compute
% step sizes (via SpatialStepSize), and remove outliers. All measurements are concatenated
% and the step‐sizes are converted to physical units using pixel_size.
%
% Finally, if the optional "symmetrize" flag is 'on', the data are mirrored about both axes. 
% The function also extracts (for diagnostic purposes) the mean localization precision from the filted data.
%
% Inputs:
% Store_track - Cell array of tracking data matrices for each ROI. Each matrix must
...
% fields 'goodfit' (logical vector) and 'widthcCI' (numeric vector)
% Index - Cell array where each element is a vector of cell IDs for that ROI.
% pixel_size - Scalar (positive) for converting pixel units to physical units.
% min_steps - Minimum number of steps required for a track (e.g., 3).
% gap_y - Bin width in the y–direction for histogram2 (e.g., 1/20).
% aspt_ratio - Aspect ratio (x/y) for normalization (e.g., 2).
% thres_ratio - Threshold ratio for filtering low–count bins (e.g., 0.1).
% uncertainty - Maximum acceptable width confidence interval (CI) value (e.g., 2.5).
% fig - 'on' or 'off'; whether to produce figures.
% symmetrize - 'on' or 'off'; if 'on', forces symmetry by mirroring the data.
% clr_map - Colormap to be used for plotting (e.g., 'jet').
%
% Outputs:
% raw_spatialDisp - [N x 3] matrix containing normalized (x,y) coordinates and step sizes
% in physical units.
% Map - The final grid (heat map) of average displacement, where bins with low counts have been set to NaN.
% mean_loc_prec - Mean localization precision (in pixels) computed from the fits.
% Count_Map - Grid map with number of steps from each specific bin.
%
% Example:
% [rawDisp, hm, mlp] = Spatial_Displacement_withFilter(Store_track, Store_mask, Store_fits, ...
% Index, 49, 3, 1/20, 2, 0.1, 2.5, 'on', 'on', 'jet');
%
% Author: Xiaofeng Dai
% Date: 05/06/2025

%% 1. Set Default Parameters for Optional Inputs
% We assign defaults if fewer than 13 inputs are given.
if nargin < 13 || isempty(clr_map),    clr_map = 'jet';           end
if nargin < 12 || isempty(symmetrize),  symmetrize = 'on';         end
if nargin < 11 || isempty(fig),         fig = 'on';                end
if nargin < 10 || isempty(uncertainty),   uncertainty = 2.5;         end
if nargin < 9  || isempty(thres_ratio),   thres_ratio = 0.1;         end
if nargin < 8  || isempty(aspt_ratio),      aspt_ratio = 2;            end
if nargin < 7  || isempty(gap_y),         gap_y = 1/20;              end
if nargin < 6  || isempty(min_steps),       min_steps = 3;             end
if nargin < 5
    error('At least five inputs are required: Store_track, Store_mask, Store_fits, Index, pixel_size.');
end
...
parfor i = 1:numFiles        
    ...
    %%% add localization precision threshold
