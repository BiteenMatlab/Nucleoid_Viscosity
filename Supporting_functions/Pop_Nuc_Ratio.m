function pop_nuc_ratio = Pop_Nuc_Ratio(num_dot,acc,R,L,anchor_spMap,...
    scalingWidth,scalingLength,num_pix_y,aspt_ratio,nuc_outline)
% calculate the nucleoid volume ration from the 3D region that is porjected
% to the nucleoid region in 2D. 
...
indot = inpolygon(projXY(:,1),projXY(:,2),nuc_outline(:,2),nuc_outline(:,1));

buff = dot_coord(indot==1,:);
pop_nuc_ratio = height(buff(buff(:,4)==2,:))/height(dot_coord(indot==1,:))*acc;
end
