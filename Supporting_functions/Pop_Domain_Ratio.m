function pop_domain_ratio = Pop_Domain_Ratio(num_dot,acc,R,L,anchor_spMap,...
    scalingWidth,scalingLength,num_pix_y,aspt_ratio,nuc_outline,DomainCoord)
...
Domdot = cell(length(DomainCoord),1);
for i=1:length(DomainCoord)
    test = DomainCoord{i};
    Domdot{i} = inpolygon(projXY(:,1),projXY(:,2),test(:,1),test(:,2));
end
domdot = cat(2,Domdot{:});
domdot = sum(domdot,2);
clear Domdot i test

nucdot = inpolygon(projXY(:,1),projXY(:,2),nuc_outline(:,2),nuc_outline(:,1));

buff_domain = dot_coord(domdot==1&nucdot==1,:);

pop_domain_ratio = height(buff_domain(buff_domain(:,4)==2,:))/height(buff_domain)*acc;
