...
- Toolboxes needed: `Parallel Computing Toolbox`, `Statistics and Machine Learning Toolbox`, `Optimization Toolbox`, `Image Processing Toolbox`, `Curve Fitting Toolbox`,`Signal Processing Toolbox`
...
The pipeline expects paired mask and tracking files for each movie:
...
Results from this pipeline are stored in folder  `<dataset>\Result_yyyy_MM_dd`.
...
- `num_pix_y = 20` (grid number in short-axis for heatmap results)
...
- `Reconstructed Geometry 2D`
  - Blue dashed line indicates cell wall, and blue solid line indicates inner membrane (the boundary that nanocage is able to reach) 
...
- `Reconstructed Geometry 3D`
  - The blue and purple solid lines were used to reconstruct the 3D geometry
...
- Weights for similarity scoring: `score_w1 = 0.5`, `score_w2 = -120` 
...
Loads:
- `UnpackResults(result_folder)` (loads variables from `Results.mat`)
...
Purpose:
- Compute viscosity in a selected macrodomain using macrodomain masks from ParB locus tracking results
...
Purpose:
- Bootstrap uncertainty analysis for accessibility and viscosity outputs

Loads:
- `Results4Btstrp(result_folder)` to load data and indices from `Results.mat` and `RawData.mat`
...
If you close MATLAB after ``3.5 DynamicSimu.mlx``, resume with:
...
- Some scripts use hard-coded paths (e.g. macrodomain boundary). Change those paths to your environment before running.
